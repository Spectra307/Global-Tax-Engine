...
export async function POST({ request }) {
  try {
    ...
    // Divider
    page.drawRectangle({ x: 35, y: y, width: 525, height: 1, color: rgb(0.9, 0.9, 0.9) });
    y -= 30;
    ...
    return new Response(pdfBytes, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="tax-invoice-${invoiceNum}.pdf"`
      }
    });
  } catch (err) {
    console.error('Invoice generation error:', err);
    throw error(500, 'Failed to generate invoice: ' + err.message);
  }
}
